...
import { estimateIncomeTaxLocal } from "./income";
...
describe("estimateIncomeTax — Grundtarif (single)", () => {
  it("ESt = 0 bei Einkommen unter Grundfreibetrag", () => {
    const r = estimateIncomeTaxLocal(1_000_000, "single", 0); // 10.000 €
    expect(r.est).toBe(0);
  ...
  it("ESt = 0 bei genau Grundfreibetrag 11.604 €", () => {
    expect(estimateIncomeTaxLocal(1_160_400, "single", 0).est).toBe(0);
  });
  ...
  it("Zone 2 — Einstiegsprogression bei 15.000 €", () => {
    ...
    expect(estimateIncomeTaxLocal(1_500_000, "single", 0).est).toBe(58_100);
  });
  ...
  it("Zone 3 — lineare Progression bei 50.000 €", () => {
    ...
    expect(estimateIncomeTaxLocal(5_000_000, "single", 0).est).toBe(1_090_600);
  });

  it("Zone 4 — 42 % Spitzensteuersatz bei 100.000 €", () => {
    // ESt = 0.42 * 100000 - 10602.13 = 31.397,87 → floor 31397
    expect(estimateIncomeTaxLocal(10_000_000, "single", 0).est).toBe(3_139_700);
  });

  it("Zone 5 — Reichensteuer 45 % bei 300.000 €", () => {
    // ESt = 0.45 * 300000 - 18936.88 = 116.063,12 → floor 116063
    expect(estimateIncomeTaxLocal(30_000_000, "single", 0).est).toBe(11_606_300);
  });
});

describe("estimateIncomeTax — Splittingtarif (married)", () => {
  it("Doppeltes Grundfreibetrag-Niveau (23.208 €)", () => {
    expect(estimateIncomeTaxLocal(2_320_800, "married", 0).est).toBe(0);
  });

  it("Bei 100.000 € Splittingtarif < Einzeltarif", () => {
    const splitting = estimateIncomeTaxLocal(10_000_000, "married", 0).est;
    const single = estimateIncomeTaxLocal(10_000_000, "single", 0).est;
...
describe("estimateIncomeTax — Solidaritätszuschlag", () => {
  it("Soli = 0 unter Freigrenze (50k Einkommen single)", () => {
    ...
    expect(estimateIncomeTaxLocal(5_000_000, "single", 0).soli).toBe(0);
  });
  ...
  it("Soli > 0 in Milderungszone (100k single)", () => {
    ...
    // min(1578, 1726) = 1578
    expect(estimateIncomeTaxLocal(10_000_000, "single", 0).soli).toBe(157_800);
  });
  ...
  it("Voller Soli (5,5 %) bei sehr hohem Einkommen", () => {
    ...
    expect(estimateIncomeTaxLocal(30_000_000, "single", 0).soli).toBe(638_300);
  });
});

describe("estimateIncomeTax — Kirchensteuer", () => {
  it("KiSt 9 % auf ESt", () => {
    const r = estimateIncomeTaxLocal(5_000_000, "single", 0.09);
    expect(r.kist).toBe(Math.floor(10_906 * 0.09) * 100);
  });

  it("KiSt 8 % auf ESt", () => {
    const r = estimateIncomeTaxLocal(5_000_000, "single", 0.08);
    expect(r.kist).toBe(Math.floor(10_906 * 0.08) * 100);
  });

  it("KiSt = 0 wenn rate = 0", () => {
    expect(estimateIncomeTaxLocal(5_000_000, "single", 0).kist).toBe(0);
  });
});

describe("estimateIncomeTax — total", () => {
  it("total = est + soli + kist", () => {
    const r = estimateIncomeTaxLocal(10_000_000, "single", 0.09);
