<script lang="ts">
  ...
  import { ArrowLeft, Plus, Trash2, CalendarRange, FileText, X, Package, GripVertical, ChevronUp, ChevronDown } from "@lucide/svelte";
    ...
    if (mode === "edit" && params?.id) {
      ...
      getOffer(numId)
        .then((res) => {
          ...
          if (res.offer.skontoPercent != null && res.offer.skontoDays != null) {
            skontoEnabled = true;
  ...
  function addFromCatalog(it: CatalogItem) {
    const vatExemptLocal = !!settings?.isKleinunternehmer || isReverseCharge;
    const vatRate = vatExemptLocal ? 0 : it.defaultVatRate;
  ...
  function moveItem(from: number, to: number) {
    if (readOnly || from === to || to < 0 || to >= items.length) return;
    const next = [...items];
  ...
  async function onSubmit(e: SubmitEvent) {
    ...
    for (const it of items) {
      if (it.periodStartIso && it.periodEndIso) {
        if (fromIsoDate(it.periodEndIso) < fromIsoDate(it.periodStartIso)) {
    ...
    <section>
      <div class="flex items-center justify-between mb-3">
        ...
        <div class="inline-flex items-center gap-2">
          <Button type="button" size="sm" variant="outline" onclick={() => (catalogPickerOpen = true)} disabled={readOnly}>
            <Package />
            Aus Katalog…
          </Button>
                ...
                <td class="px-2 py-2 align-top">
                  <div class="flex items-center gap-1">
                    <button
                      ...
                      ondragend={onDragEnd}
                      disabled={readOnly}
                      class="cursor-grab active:cursor-grabbing text-muted-foreground/40 hover:text-foreground disabled:opacity-30 disabled:cursor-default transition-colors"
                ...
                <td class="px-2 py-1.5">
                  ...
                  <div class="mt-1.5 flex flex-wrap gap-1.5">
                    ...
                    {#if it.showDetail}
                      <button
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md border border-input bg-transparent px-2 h-7 text-xs text-muted-foreground hover:bg-accent hover:text-destructive transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                        onclick={() => {
                          it.showDetail = false;
                          it.longDescription = "";
                          items = [...items];
                    ...
                    {:else}
                      <button
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md border border-input bg-transparent px-2 h-7 text-xs text-muted-foreground hover:bg-accent hover:text-foreground transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                        onclick={() => {
                          it.showDetail = true;
                          items = [...items];
                        }}
      ...
      <CardContent class="space-y-4">
        ...
        <div class="border-t pt-4">
          <label class="flex items-center gap-2 text-sm cursor-pointer select-none">
            <Checkbox bind:checked={skontoEnabled} disabled={readOnly} />
          ...
          {#if skontoEnabled}
            <div class="mt-3 ml-7 grid grid-cols-2 gap-3 max-w-md">
              ...
              <p class="col-span-2 text-xs text-muted-foreground">
                Erscheint als Hinweis auf der Angebots-PDF und wird auf die abgeleitete Rechnung übernommen.
              </p>
            </div>
          {/if}
        </div>
